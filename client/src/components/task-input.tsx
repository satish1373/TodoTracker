          ...
          <div>
            {filteredTasks.map(task => (
              <div key={task.id} className="flex items-center justify-between p-2 border-b">
                <span className={`flex-1 ${task.completed ? "line-through" : ""}`} dangerouslySetInnerHTML={{ __html: task.text.replace(new RegExp(`(${searchTerm})`, 'gi'), (match) => `<span class="bg-yellow-200">${match}</span>`) }} />
